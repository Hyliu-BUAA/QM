 ...
 * @LastEditTime: 2022-06-16 11:57:54
...
2. Inner product: $$ \braket{\phi|\psi}^\dagger = \braket{\psi|\phi} $$
    - Note that $\braket{\phi|\psi}$ is a scaler, so the `Hermitian conjugate` is just the `complex conjugate`$$ \braket{\phi|\psi}^\dagger = \braket{\psi|\phi}^* $$
